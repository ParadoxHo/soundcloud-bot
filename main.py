...
class UniversalMusicBot:
            ...
            try:
                with open(file_path, 'rb') as audio_file:
                    await context.bot.send_audio(
                        ...
                        caption=self._format_caption(track),
                        parse_mode='HTML'
                    )
    ...
    def _format_caption(self, track: dict) -> str:
        """Форматирует подпись для аудио - ТОЛЬКО название и длительность"""
        return f"🎵 <b>{track.get('title', 'Неизвестный трек')}</b>\n⏱️ {self.format_duration(track.get('duration'))}"
    ...
    def run(self):
        ...
        print('🎵 Минималистичные сообщения: только трек и длительность')
