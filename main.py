...
MAX_FILE_SIZE_MB = 45  # Ограничение до 45 MB для бесплатного Railway
...
# МЕДЛЕННЫЕ настройки только для файлов до 45MB
LARGE_FILE_OPTS = {
    'format': 'bestaudio[ext=m4a]/bestaudio[ext=mp3]/bestaudio/best',
...
class StableMusicBot:
    ...
    def validate_track_fast(self, track_info: dict) -> bool:
        ...
        # Основано на приблизительной оценке: 1 минута ≈ 1 MB для 128kbps
    ...
    async def download_and_send_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        ...
        try:
            if file_size_mb > 25:  # Для файлов больше 25MB используем медленный метод
...
Выберите настройку для изменения:"""
    ...
    def run(self):
        print('🚀 Запуск оптимизированного Music Bot с поддержкой файлов до 45MB для Railway...')
        ...
        print('✅ Оптимизированный бот запущен! Максимальный размер файлов: 45 MB для бесплатного Railway.')
        app.run_polling()
