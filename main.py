...
class StableMusicBot:
    ...
    async def download_large_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        ...
        try:
            if status_message:
                await status_message.edit_text(f"⬇️ Скачиваем большой файл...\n🎵 {track.get('title', 'Неизвестный треk')[:30]}")
            else:
                # Создаем новое сообщение о статусе если его нет
                query = update.callback_query
                if query:
                    status_message = await query.message.reply_text(f"⬇️ Скачиваем большой файл...\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
            ...
            if not files:
                logger.error(f"❌ Файлы не были скачаны для: {track.get('title')}")
                if status_message:
                    await status_message.edit_text(f"❌ Не удалось скачать файл\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
                return False
            ...
            if not audio_file:
                logger.error(f"❌ Все файлы слишком маленькие: {files}")
                if status_message:
                    await status_message.edit_text(f"❌ Файл слишком маленький\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
                return False
