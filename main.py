...
DOWNLOAD_TIMEOUT = int(os.environ.get('DOWNLOAD_TIMEOUT', 120))  # Уменьшили таймаут
SEARCH_TIMEOUT = int(os.environ.get('SEARCH_TIMEOUT', 20))       # Уменьшили таймаут
REQUESTS_PER_MINUTE = int(os.environ.get('REQUESTS_PER_MINUTE', 10))

# Ускоренные настройки для SoundCloud
SOUNDCLOUD_OPTS = {
    'format': 'bestaudio[ext=mp3]/bestaudio[ext=m4a]/bestaudio/best',  # Приоритет готовых форматов
    'outtmpl': os.path.join(tempfile.gettempdir(), '%(id)s.%(ext)s'),
    'quiet': True,
    'no_warnings': True,
    'retries': 2,                    # Уменьшили ретраи
    'fragment_retries': 2,           # Уменьшили ретраи
    'skip_unavailable_fragments': True,
    'noprogress': True,
    'nopart': True,                  # Отключили частичные загрузки
    'noplaylist': True,
    'max_filesize': MAX_FILE_SIZE_MB * 1024 * 1024,
    'ignoreerrors': True,
    'socket_timeout': 20,            # Уменьшили таймаут
    'extractaudio': True,
    'audioformat': 'best',           # Без конвертации
}
...
class UniversalMusicBot:
    def __init__(self):
        self.download_semaphore = asyncio.Semaphore(2)  # Увеличили до 2 одновременных скачиваний
...
👤 {user.mention_html()}"""
        ...
        # Добавляем подсказки для следующих действий
        caption += f"\n\n💡 <i>Еще музыка: /random или напиши \"найди [исполнитель]\"</i>"
    ...
    async def find_track(self, query: str):
        ...
        async with self.search_semaphore:
            ydl_opts = {
                ...
                'ignoreerrors': True,
                'noplaylist': True,
                'socket_timeout': 15,  # Уменьшили таймаут
            }
            ...
            try:
                ...
                def perform_search():
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        return ydl.extract_info(f"scsearch8:{query}", download=False)  # Уменьшили до 8 для скорости
