...
class UniversalMusicBot:
    ...
    async def handle_find_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE, message_text: str):
        ...
        try:
            ...
            # Генерируем пожелание и отправляем аудио
            wish = await self.ai.generate_wish(original_query, track['title'], track.get('artist'))
    ...
    async def handle_random_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Генерируем пожелание
            wish = await self.ai.generate_wish(random_query, track['title'], track.get('artist'))
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print('💡 Решение: Убедитесь, что не запущено других инстансов бота с тем же токеном.')
                    print('💡 На Railway: Проверьте, что не запущено несколько контейнеров одновременно.')
                    sys.exit(1)
