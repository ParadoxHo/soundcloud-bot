...
# НАСТРОЙКИ БЕЗ КОНВЕРТАЦИИ - ТОЛЬКО ГОТОВЫЕ MP3
ULTRA_FAST_DOWNLOAD_OPTS = {
    # ТОЛЬКО ГОТОВЫЕ MP3 ФАЙЛЫ БЕЗ КОНВЕРТАЦИИ
    'format': 'bestaudio[ext=mp3][filesize<45M]/bestaudio/best',
    
    'outtmpl': os.path.join(tempfile.gettempdir(), '%(id)s.%(ext)s'),
    'quiet': True,
    'no_warnings': True,
    
    # ВАЖНО: ОТКЛЮЧАЕМ КОНВЕРТАЦИЮ!
    'extractaudio': False,
    'keepvideo': False,
    
    # ОТКЛЮЧАЕМ POSTPROCESSORS КОТОРЫЕ ТРЕБУЮТ FFMPEG/FFPROBE
    'postprocessors': [],
...
class StableMusicBot:
    ...
    async def download_and_send_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict) -> bool:
        """Основной метод скачивания - ТОЛЬКО ГОТОВЫЕ MP3"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False

        try:
            return await self.download_mp3_simple(update, context, track)
        except Exception as e:
            logger.exception(f'Ошибка скачивания трека: {e}')
            return False

    async def download_mp3_simple(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict) -> bool:
        """Простое скачивание готовых MP3 файлов"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False

        # ПРОСТЫЕ НАСТРОЙКИ БЕЗ КОНВЕРТАЦИИ
        ydl_opts = {
            # ТОЛЬКО ГОТОВЫЕ MP3
            'format': 'bestaudio[ext=mp3][filesize<45M]/bestaudio/best',
            'outtmpl': os.path.join(tempfile.gettempdir(), '%(id)s.%(ext)s'),
        ...
        try:
                    ...
                    if os.path.isfile(fpath) and fname.lower().endswith('.mp3'):
                        ...
                        if file_size_mb > MAX_FILE_SIZE_MB:
                            await context.bot.send_message(
                                ...
                                text=f'❌ Файл слишком большой для отправки в Telegram ({file_size_mb:.1f} MB)'
                            )
                            return False
            ...
            logger.exception(f'Ошибка простого скачивания MP3: {e}')
            return False
