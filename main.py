...
class FreeUploadManager:
    ...
    async def _upload_transfersh(self, file_data: bytes, filename: str) -> str:
        ...
        response = requests.put(
            f'https://transfer.sh/{quote(filename)}',
            data=file_data,
            timeout=30,
...
class MusicBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        user_data = self.db.get_user_data(user_id)
        user_data['stats']['first_seen'] = datetime.now().strftime('%d.%m.%Y %H:%M')
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        help_text = (
            ...
            "• /preferences - музыкальные предпочтения\n"
        )
    ...
    async def handle_search(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Выполняем поиск
            search_results = await self._perform_soundcloud_search(query, user_data)
            ...
            if query not in user_data['search_history']:
                ...
                user_data['search_history'] = user_data['search_history'][:50]  # Ограничиваем историю
    ...
    async def _perform_soundcloud_search(self, query: str, user_data: dict) -> list:
        ...
        ydl_opts = {
            ...
            'no_warnings': True,
            'extract_flat': True,
            'default_search': f'scsearch{10}:{query}',
        }
        
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(f"scsearch{10}:{query}", download=False)
                ...
                if 'entries' in info:
                    ...
                    for entry in info['entries']:
                        if entry:
                            ...
                            track_info = await self._get_track_details(entry['url'])
                            
                            results.append({
                                'id': entry.get('id', entry['url']),
                                ...
                                'likes': track_info.get('like_count', 0),
    ...
    async def _get_track_details(self, url: str) -> dict:
    ...
    async def _show_search_results(self, update: Update, user_data: dict, search_msg=None):
        ...
        for i, result in enumerate(page_results, start=1):
            ...
            # Добавляем информацию о лайках и прослушиваниях если есть
            if result.get('likes') or result.get('play_count'):
                stats = []
                if result.get('likes'):
                    stats.append(f"❤️ {result['likes']}")
                if result.get('play_count'):
                    stats.append(f"🎧 {result['play_count']}")
                if stats:
                    text += f"   {' | '.join(stats)}\n"
    ...
    async def _download_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track_id: str):
        ...
        status_msg = await update.callback_query.message.reply_text("⬇️ Подготовка трека с SoundCloud...")
        
        try:
            # Получаем информацию о треке для определения размера
            ydl_info = {
            ...
            # Выбираем стратегию скачивания в зависимости от размера
            if file_size < 20 * 1024 * 1024:  # До 20MB
                await status_msg.edit_text("⬇️ Скачивание...")
                await self._download_small_file(update, context, track, status_msg)
            else:  # Большие файлы через облако
                await status_msg.edit_text("⬇️ Обработка...")
                await self._download_large_file(update, context, track, status_msg)
            ...
            user_data['download_history'] = user_data['download_history'][:100]  # Ограничиваем историю
    ...
    async def _download_small_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_msg):
        """Скачивание небольших файлов напрямую в память"""
        try:
            ydl_opts = {
                'format': 'bestaudio[ext=m4a]/bestaudio',
    ...
    async def _download_large_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_msg):
        """Скачивание больших файлов через облачные сервисы"""
        try:
            # Скачиваем трек
            ydl_opts = {
                'format': 'bestaudio[ext=m4a]/bestaudio',
            ...
            await status_msg.edit_text("⬇️ Финальная подготовка...")
            
            # Загружаем в облако
            filename = f"{track['title'][:50]}.m4a"
            file_url = await self.uploader.upload_file(audio_data, filename)
            
            # Отправляем через прямую ссылку
            await context.bot.send_audio(
                chat_id=update.effective_chat.id,
                audio=file_url,
                title=track['title'][:64],
            ...
            await status_msg.edit_text("❌ Ошибка при обработке большого файла")
            raise e
    ...
    async def show_favorites(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard.append([InlineKeyboardButton("🗑 Очистить все", callback_data="clear_favorites")])
    ...
    async def show_history(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for i, record in enumerate(history[:10], 1):  # Показываем последние 10
    ...
    async def show_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if stats.get('last_search'):
            last_search = datetime.fromisoformat(stats['last_search']).strftime('%d.%m.%Y %H:%M')
            text += f"🔍 Последний поиск: <b>{last_search}</b>\n"
        
        if stats.get('last_download'):
            last_download = datetime.fromisoformat(stats['last_download']).strftime('%d.%m.%Y %H:%M')
            text += f"💾 Последняя загрузка: <b>{last_download}</b>"
    ...
    async def run(self):
        ...
        webhook_url = f"https://{os.environ.get('RAILWAY_STATIC_URL', 'localhost')}.railway.app/{BOT_TOKEN}"
