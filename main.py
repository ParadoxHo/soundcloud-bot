...
MAX_CONCURRENT_DOWNLOADS = 1  # Только 1 скачивание одновременно
DOWNLOAD_TIMEOUT = 180  # 3 минуты таймаут на скачивание
SEARCH_TIMEOUT = 30  # 30 секунд на поиск

# НАДЕЖНЫЕ НАСТРОЙКИ СКАЧИВАНИЯ
STABLE_DOWNLOAD_OPTS = {
    'format': 'bestaudio[filesize<45M]/bestaudio/best',
    'outtmpl': os.path.join(tempfile.gettempdir(), '%(id)s.%(ext)s'),
    'quiet': True,
    'no_warnings': True,
    
    # ПРОСТАЯ КОНВЕРТАЦИЯ В MP3
    'postprocessors': [{
...
class StableMusicBot:
    def __init__(self):
        ...
        self.search_semaphore = asyncio.Semaphore(3)  # Ограничение поисковых запросов
        ...
        async with self.download_semaphore:
            try:
                return await asyncio.wait_for(
                    self.download_with_ffmpeg(update, context, track),
    ...
    async def download_with_ffmpeg(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict) -> bool:
        """ИСПРАВЛЕННОЕ скачивание с конвертацией в MP3 через ffmpeg"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ydl_opts = STABLE_DOWNLOAD_OPTS.copy()
                ...
                if not mp3_files:
                    # Если MP3 не создался, ищем исходный аудио файл
                    audio_files = [f for f in os.listdir(tmpdir) 
                                 if any(f.lower().endswith(ext) for ext in ['.mp3', '.m4a', '.webm', '.ogg', '.wav', '.mp4'])]
                    
                    if not audio_files:
                        logger.error("❌ Аудио файл не найден после скачивания")
                        return False
                    
                    # Используем первый найденный аудио файл
                    ...
                    logger.info(f"✅ Трек успешно отправлен: {mp3_files[0]} ({actual_size_mb:.1f} MB)")
                    return True
    ...
    # ==================== ОСТАЛЬНЫЕ МЕТОДЫ (сохранены все функции) ====================
    ...
    async def update_charts_cache(self):
        ...
        for query in POPULAR_SEARCHES[:3]:  # Уменьшили количество запросов
            try:
                results = await self.search_soundcloud(query, album_only=False)
                if results:
                    charts_data[query] = results[:6]  # Уменьшили количество результатов
                await asyncio.sleep(1)  # Увеличили задержку
            except Exception as e:
    ...
    async def show_charts(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            top_tracks = all_tracks[:20]  # Уменьшили количество треков
        ...
        try:
            ...
            for query in playlist['queries'][:2]:  # Уменьшили количество запросов
                try:
                    results = await self.search_soundcloud(query, album_only=False)
                    if results:
                        all_tracks.extend(results[:6])  # Уменьшили количество результатов
                    await asyncio.sleep(1)  # Увеличили задержку
                except Exception as e:
            ...
            random.shuffle(all_tracks)
            playlist_tracks = all_tracks[:20]  # Уменьшили размер плейлиста
