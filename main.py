...
class DeepSeekIntegration:
    def __init__(self, api_key: str = None):
        ...
        if self.enabled:
            ...
            print("ℹ️ DeepSeek API ключ не найден, используются стандартные методы")
    ...
    async def enhance_search_query(self, original_query: str) -> str:
        """Улучшает поисковый запрос через DeepSeek"""
        if not self.enabled:
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(f"🤖 DeepSeek улучшил запрос: '{original_query}' -> '{enhanced_query}'")
                    
                    # Если ИИ вернул пустую строку или слишком длинный запрос, используем оригинальный
                    ...
                    error_text = await response.text()
                    print(f"❌ Ошибка DeepSeek API при улучшении запроса: {response.status} - {error_text}")
...
class UniversalMusicBot:
    ...
    async def handle_find_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE, message_text: str):
        ...
        try:
            ...
            # Этап 1: Улучшение запроса через ИИ
            enhanced_query = original_query
            if self.deepseek.enabled:
                await status_msg.edit_text(f"{status_text}\n⏳ Этап 1/4: Улучшаем запрос с ИИ...", parse_mode='HTML')
                try:
                    enhanced_query = await asyncio.wait_for(
                        self.deepseek.enhance_search_query(original_query),
                        timeout=3.0
                    )
                    if enhanced_query != original_query:
                        print(f"🎯 Используем улучшенный запрос: {enhanced_query}")
                except asyncio.TimeoutError:
                    print("⏰ Таймаут улучшения запроса, используем оригинальный")
                except Exception as e:
                    print(f"❌ Ошибка улучшения запроса: {e}")
            else:
            ...
            wish_task = asyncio.create_task(
                self.deepseek.generate_wish(original_query, track['title'], track.get('artist'))
            )
            ...
            try:
                with open(file_path, 'rb') as audio_file:
                    ...
                    # Добавляем информацию об улучшенном запросе, если он изменился
                    search_info = ""
                    if enhanced_query != original_query and self.deepseek.enabled:
                        search_info = f"\n🔍 Улучшенный запрос: <code>{enhanced_query}</code>\n"
                    ...
                    await context.bot.send_audio(
                        ...
                        caption=f"🎵 <b>{track.get('title', 'Неизвестный трек')}</b>\n⏱️ {self.format_duration(track.get('duration'))}{search_info}\n\n{wish}",
                        parse_mode='HTML'
                    )
            ...
            try:
                ...
                final_text = f"✅ Найдено: <b>{track.get('title', 'Неизвестный трек')}</b>\n⏱️ {self.format_duration(track.get('duration'))}"
                if enhanced_query != original_query and self.deepseek.enabled:
                    final_text += f"\n🔍 Улучшенный запрос: <code>{enhanced_query}</code>"
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        ai_features = []
        if self.deepseek.enabled:
            ai_features.append("🤖 AI-улучшение запросов")
            ai_features.append("💝 AI-пожелания")
        else:
            ai_features.append("⚡ стандартный поиск")
        
        ai_status = " + ".join(ai_features)
    ...
    def run(self):
        ...
        print('📊 Поэтапный статус: улучшение запроса → поиск → скачивание → отправка')
        
        if self.deepseek.enabled:
            print('🤖 DeepSeek AI: улучшение поисковых запросов + персонализированные пожелания')
        else:
