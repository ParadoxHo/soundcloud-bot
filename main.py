...
SEARCH_TIMEOUT = 15  # Уменьшили таймаут поиска
...
FAST_DOWNLOAD_OPTS = {
    ...
    'quiet': True,
    'no_warnings': True,
    'retries': 1,  # Уменьшили повторные попытки
    'fragment_retries': 1,
    ...
    'audioformat': 'mp3',
    'audioquality': '0',
    'concurrent_fragment_downloads': 3,  # Параллельные фрагменты
}
...
LARGE_FILE_OPTS = {
    ...
    'ignore_no_formats_error': True,
    'socket_timeout': 15,
    'buffersize': 524288,  # Уменьшили буфер
    'http_chunk_size': 5242880,  # Уменьшили размер чанка
    'extractaudio': True,
    'audioformat': 'mp3',
    'audioquality': '0',
...
FAST_INFO_OPTS = {
    ...
    'noplaylist': True,
    'extract_flat': True,
    'socket_timeout': 8,  # Уменьшили таймаут
    'ignoreerrors': True,
}
...
class StableMusicBot:
    def __init__(self):
        ...
        self.search_semaphore = asyncio.Semaphore(5)  # Увеличили параллелизм поиска
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    await asyncio.sleep(0.5)  # Уменьшили время ожидания
    ...
    async def search_soundcloud(self, query: str, album_only: bool = False, user_id: str = None):
        ...
        async with self.search_semaphore:
            ...
            try:
                def perform_search():
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        return ydl.extract_info(f"scsearch20:{query}", download=False)  # Уменьшили количество результатов
...
Выберите настройку для изменения:"""
    ...
    async def get_popular_recommendations(self, limit: int = 15, user_id: str = None):
        ...
        for query in POPULAR_SEARCHES[:3]:  # Уменьшили количество запросов
            try:
                results = await self.search_soundcloud(query, user_id=user_id)
                if results:
                    popular_tracks.extend(results[:6])  # Уменьшили количество результатов
            except Exception as e:
    ...
    async def show_recommendations(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            recommendations = await self.get_recommendations(user.id, 25)  # Уменьшили лимит
    ...
    async def update_charts_cache(self, user_id: str = None):
        ...
        if last_update:
            ...
            if now - last_update_date < timedelta(hours=6):  # Уменьшили время кэша
                return

        logger.info("🔄 Обновление кэша чартов...")

        charts_data = {}
        for query in POPULAR_SEARCHES[:4]:  # Уменьшили количество запросов
            try:
                results = await self.search_soundcloud(query, user_id=user_id)
                if results:
                    charts_data[query] = results[:8]  # Уменьшили количество результатов
                await asyncio.sleep(0.5)  # Уменьшили задержку
            except Exception as e:
    ...
    async def show_charts(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            top_tracks = all_tracks[:25]  # Уменьшили количество треков
    ...
    async def generate_playlist(self, update: Update, context: ContextTypes.DEFAULT_TYPE, playlist_id: str):
        ...
        try:
            all_tracks = []
            for query in playlist['queries'][:2]:  # Уменьшили количество запросов
                try:
                    results = await self.search_soundcloud(query, user_id=str(user.id))
                    if results:
                        all_tracks.extend(results[:8])  # Уменьшили количество результатов
                    await asyncio.sleep(0.5)  # Уменьшили задержку
                except Exception as e:
            ...
            random.shuffle(all_tracks)
            playlist_tracks = all_tracks[:25]  # Уменьшили количество треков
