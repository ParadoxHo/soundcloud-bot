...
# МАКСИМАЛЬНО БЫСТРЫЕ НАСТРОЙКИ СКАЧИВАНИЯ В MP3
ULTRA_FAST_DOWNLOAD_OPTS = {
    # ПРИОРИТЕТ ФОРМАТОВ КОТОРЫЕ ЛЕГКО КОНВЕРТИРУЮТСЯ В MP3
    ...
    # КОНВЕРТАЦИЯ В MP3 С ОПТИМИЗАЦИЕЙ
    'extractaudio': True,
    'audioformat': 'mp3',
    'audioquality': '5',
    
    # POSTPROCESSOR ДЛЯ НАДЕЖНОЙ КОНВЕРТАЦИИ
    'postprocessors': [{
...
class StableMusicBot:
    ...
    async def get_file_size_fast(self, url: str) -> float:
        """Быстрое получение размера файла"""
    ...
    async def download_and_send_track_fast(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict) -> bool:
        """Оптимизированное скачивание и отправка трека в MP3"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False

        try:
            # БЫСТРОЕ СКАЧИВАНИЕ С КОНВЕРТАЦИЕЙ В MP3
            return await self.download_mp3_direct(update, context, track)
        except Exception as e:
            logger.exception(f'Ошибка скачивания трека: {e}')
            return await self.download_fallback_mp3(update, context, track)

    async def download_mp3_direct(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict) -> bool:
        """Прямое скачивание в MP3"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False
        ...
        try:
            ...
            logger.exception(f'Ошибка прямого скачивания MP3: {e}')
            return False

    async def download_fallback_mp3(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict) -> bool:
        """Резервный метод скачивания в MP3"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False

        # УПРОЩЕННЫЕ НАСТРОЙКИ ДЛЯ СЛОЖНЫХ СЛУЧАЕВ
        ydl_opts = {
            'format': 'bestaudio[filesize<45M]/bestaudio/best',
            'outtmpl': os.path.join(tempfile.gettempdir(), '%(id)s.mp3'),
            'quiet': True,
            'no_warnings': True,
            'extractaudio': True,
            'audioformat': 'mp3',
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                for fname in os.listdir(tmpdir):
                    ...
                    if os.path.isfile(fpath) and os.path.getsize(fpath) > 0:
                        ...
                        # Принудительно переименовываем в .mp3 если нужно
            ...
            logger.exception(f'Ошибка резервного скачивания: {e}')
            return False
    ...
    async def get_file_size(self, url: str) -> float:
        """Основной метод получения размера"""
